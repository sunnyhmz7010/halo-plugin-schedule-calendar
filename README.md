...
### ☁️ Google Calendar 只读订阅

- 支持在插件设置中添加 Google Calendar 等兼容 iCal / ICS 的外部日历订阅地址
- 外部日历事项会自动并入前台周视图、当前状态、下一个事项摘要、Finder 和公开 REST API
- 外部日历事件当前为只读展示，不会进入本地事项 CRUD 或编辑器卡片管理
...
- 提供公开 REST API
- Finder 和 REST 返回的数据围绕同一套周视图、摘要和事项模型组织，并可包含外部 ICS 订阅事件
...
如果你在插件设置里修改了前台访问路径，请改用新的路径访问公开页面。
